...
@RestController
public class FileUploaderController implements FileUploaderControllerApi, ReadFace64InGridFsApi {
    ...
    public GraceJSONResult uploadFace(String userId,
        ...
        if (file != null) {
            ...
            if (StringUtils.isNotBlank(fileName)) {
                ...
                // 执行上传
                path = uploaderService.uploadOBS(file, userId, suffix);
        ...
        if (StringUtils.isNotBlank(path)) {
            finalPath = fileResource.getObsHost() + path;
        } else {
...
//                    finalPath = fileResource.getOssHost() + path;
    ...
    private File readGridFSByFaceId(String faceId) throws Exception {
        ...
        File fileTemp = new File("/workspaces/temp_face");
        if (!fileTemp.exists()) {
            fileTemp.mkdirs();
        }

        File myFile = new File("/workspaces/temp_face/" + fileName);
