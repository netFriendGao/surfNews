...
import com.imooc.api.controller.admin.ReadFace64InGridFsApi;
...
import com.imooc.utils.extend.CompareFaceByBase64Solution;
...
public class AdminMngController extends BaseController implements AdminMngControllerApi {
    ...
    @Autowired
    private CompareFaceByBase64Solution compareFaceByBase64Solution;

    @Autowired
    private FaceVerifyUtils faceVerifyUtils;

    @Autowired
    private ReadFace64InGridFsApi fileUploaderControllerApi;
    ...
    public GraceJSONResult adminFaceLogin(AdminLoginBO adminLoginBO,
        ...
        GraceJSONResult bodyResult = null;
        try {
            bodyResult = fileUploaderControllerApi.readFace64InGridFS(adminFaceId);
        } catch (Exception e) {
            e.printStackTrace();
        }
        String base64DB = (String)bodyResult.getData();

        // 3. 调用华为云进行人脸对比识别，判断可信度，从而实现人脸登录
        boolean result = compareFaceByBase64Solution.CompareFace(tempFace64,base64DB);
        if (!result) {
            return GraceJSONResult.errorCustom(ResponseStatusEnum.ADMIN_FACE_LOGIN_ERROR);
        }
